...
async function load() {
  ...
  try {
    ...
    // se o backend tiver filtro por code, ele vai usar
...
async function removeProduct(p) {
  ...
  try {
    ...
    // se deletou o último item da página, tenta voltar 1 página
  ...
  <div class="wrap">
    <div class="top">
      ...
      <div class="actions">
        ...
        <button class="primary" @click="openCreate" :disabled="loading">+ Novo</button>
      </div>
    </div>
    ...
    <table v-else class="tbl">
      ...
      <tbody>
        <tr v-for="p in products" :key="p.id">
          ...
          <td>{{ p.price }}</td>
          <td class="btns">
            <button @click="openEdit(p)" :disabled="loading">Editar</button>
            <button class="danger" @click="removeProduct(p)" :disabled="loading">Excluir</button>
          </td>
        </tr>
