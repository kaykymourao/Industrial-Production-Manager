...
function parseBackendError(e) {
  ...
  if (data.fieldErrors) return data.error ?? "Validação falhou.";
...
async function load() {
  ...
  try {
    ...
    totalElements.value = res.data?.totalElements ?? products.value.length;
  } catch (e) {
...
async function removeProduct(p) {
  const ok = confirm(`Deletar o produto "${p.code} - ${p.name}"?`);
  if (!ok) return;
  ...
  try {
    ...
    if (products.value.length === 1 && page.value > 0) page.value -= 1;
    await load();
  } catch (e) {
    error.value = parseBackendError(e);
  } finally {
...
<template>
  <div class="container">
    <div class="header">
      <div>
        <h1>Produtos</h1>
        <p class="muted">Gerencie produtos, busque por código e mantenha o catálogo atualizado.</p>
      </div>

      <div class="actions">
        <input
          v-model="searchCode"
          placeholder="Buscar por código (ex: P-001)"
        ...
        <button class="btn primary" @click="openCreate" :disabled="loading">+ Novo</button>
      </div>
    </div>
    ...
    <div class="card section">
      ...
      <div v-else-if="products.length === 0" class="empty">
        <div class="emptyTitle">Nenhum produto encontrado</div>
        <div class="muted">Tente buscar por outro código ou clique em <b>+ Novo</b>.</div>
      </div>
      ...
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="p in products" :key="p.id">
            ...
            <td class="btns">
              <button class="btn" @click="openEdit(p)" :disabled="loading">Editar</button>
              <button class="btn danger" @click="removeProduct(p)" :disabled="loading">Excluir</button>
            </td>
          </tr>
        </tbody>
      </table>
